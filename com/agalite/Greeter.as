package com.agalite//
{
    ...
    public class PopUp extends Sprite
    {
		...
		public function pinstripe():void {
			...
			pinstripeTop.graphics.beginFill(0xCCCCCC);
			pinstripeTop.graphics.drawRect(0,0,760,30);
			pinstripeTop.graphics.endFill();
			pinstripeTop.name="up";
			pinstripeTop.alpha=.23;
			addChildAt(pinstripeTop,1);

			var pinstripeBot:Sprite=new Sprite;
			//pinstripeBot.graphics.lineStyle(0,0x000000,100);
			pinstripeBot.graphics.beginFill(0x7D796D);
			pinstripeBot.graphics.drawRect(0,0,760,400);
			pinstripeBot.graphics.endFill();
			pinstripeBot.name="up";
			pinstripeBot.alpha=.85;
			addChildAt(pinstripeBot,0);
			...
			pinstripeBot.filters = filtersArray;// The last step is to take our movie clip that we made at the beginning, so we take our object and apply the filtersArray.
